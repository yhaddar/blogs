...
<body>
    ...
    <section class="Home-bg">
        <h1 class="home-text">add blog</h1>
    </section>
    ...
    <div class="add-blog" style="width: 97%">
        ...
        <form action="{{ route('store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input placeholder="title" name="title" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                <span style="color: var(--clr-primary-5)"> @error('title') {{ $message }} @enderror </span>
            </div>
            <div class="form-floating">
                ...
                <span style="color: var(--clr-primary-5)"> @error('description') {{ $message }} @enderror </span>
            </div>
            <div class="select">
                <select  style="cursor: pointer;" class="form-select" name="category" aria-label="Default select example">
                    <option selected>Categories</option>
                    @foreach ($category as $cat)
                        <option value="{{ $cat -> category }}" style="text-transform: capitalize;">{{$cat-> category  }}</option>
                    @endforeach
                </select>
                <span style="color: var(--clr-primary-5)"> @error('category') {{ $message }} @enderror </span>
            </div>
            <div class="mb-3"></div>
            <div class="mb-3">
                <input type="file" name="image" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                <span style="color: var(--clr-primary-5)"> @error('image') {{ $message }} @enderror </span>
            </div>
